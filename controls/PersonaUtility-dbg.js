sap.ui.define(["jquery.sap.global", "sap/ui/core/Control"], function (jQuery, Control) {
				...
				oDataModel.read("/MultiplePersonaSet", {
					success: function (oData, response) {
						if (oData !== undefined && oData.results !== undefined && oData.results.length > 1) {
							...
							that._oDialog.open();
							sap.ui.core.Fragment.byId("CLMPRJ_ID", 'btnOk').attachPress(onPressOkBtn);
							sap.ui.core.Fragment.byId("CLMPRJ_ID", 'idPersonaListItem').
							attachSelectionChange(onSelectPersona);
		...
		function onSelectPersona(oEvent) {
			...
			oEvent.getSource().getParent().getParent().setModel(oContextObj, "PersonaModel");
		}

		function onPressOkBtn(oEvent) {
			var that = this;

			var serviceUrl1 = "/sap/opu/odata/sap/ZGECAGS010_SRV/";
			var oDataModel = new sap.ui.model.odata.v2.ODataModel(serviceUrl1);
			var oPersonaObj = oEvent.getSource().getParent().getModel("PersonaModel");
