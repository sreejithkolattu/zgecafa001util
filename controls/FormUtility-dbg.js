	...
	return {
		...
		ValidateForm: function (that, formId) {
			...
			for (var vStart in oControls) {
				if (isValidControl(oControls[vStart]) && isValidDate(oControls[vStart],"Start")) {
					...
					for (var vEnd in oControls) {
						if (isValidControl(oControls[vEnd]) && isValidDate(oControls[vStart],"Start") && oControls[vEnd].getId() === vFlag[0]+"dpEnd"+vFlag[1]) {
