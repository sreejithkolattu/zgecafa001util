sap.ui.define(["jquery.sap.global", "sap/ui/core/Control"], function (jQuery, Control) {
	...
	return {
		...
		ValidateForm: function (that, formId) {
				...
				try {
					...
					if (validateControl(oControl, "fgSelect")) {
						if (validateControl(oControl, "fgSelectEntry")) {
							isInvalid = setErrorStateDropDown(oControl, that);
							//isToSetFocusFlag = isToSetFocusFlag ? setFocus(oControl) : isToSetFocusFlag;
						}
			...
			//#16: Validation for multi combo box
			// The validation is similar to select box
			oControls = that.getView().getControlsByFieldGroupId("fgMultiComboBox");
			oControls.forEach(function (oControl) {
				try {
					if (partValidation && oControl.getId().indexOf(formId) === -1) {
						return;
					}
					if (validateControl(oControl, "fgMultiComboBox")) {
						if (validateControl(oControl, "fgMultiComboBoxEntry")) {
							isInvalid = setErrorState(oControl, that);
							//isToSetFocusFlag = isToSetFocusFlag ? setFocus(oControl) : isToSetFocusFlag;
						}
					} else if (oControl.getValueState() === "Error") {
	...
	function validateControl(oControl, cType) {
		...
		} else if ((cType === "fgSelect" || cType === "fgMultiComboBox" || cType === "fgSelectValue" || cType === "fgMobile" || cType ===
				"fgEmail" || cType === "fgNumber" ||
		...
		} else if (cType === "fgMultiComboBoxEntry" && isValidControl(oControl)) { // Checks : Selected index is 00 or undefined
			return (oControl.getSelectedItems.length === 0 || oControl.getSelectedKeys().length === 0 || oControl.getSelectedKeys() === undefined);
	...
	function setErrorStateDropDown(oControl, that) {
