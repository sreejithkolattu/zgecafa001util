sap.ui.define(["jquery.sap.global",
	...
	return {
		 ...
		 * @public
		 */
		_setKeepInTouchFormData: function (that, model, fragment, fragmentId) {
			// Model Instatiation
			model = (model !== null && model !== undefined) ? model : _setKeepInTouchFormModel();
			// Fragment Creation
			_handleDialog(that, model, "zgecafa001util.zgecafa001util.fragments.KeepInTouchForm", fragmentId);
	...
	function _handleDialog(that, model, fragment, fragmentId) {
		if (that._oKeepInTouchFragment !== undefined) {
			try {
				that._oKeepInTouchFragment.destroy();
				that._oKeepInTouchFragment = undefined;
			} catch (e) {}
		}
		fragmentId = (fragmentId !== "" && fragmentId !== undefined) ? fragmentId : "KeepInTouchForm_FRAG";
		that._oKeepInTouchFragment = sap.ui.xmlfragment(that.createId(fragmentId), fragment, that);
		that.getView().addDependent(that._oKeepInTouchFragment);
		jQuery.sap.syncStyleClass("sapUiSizeCompact", that.getView(), that._oKeepInTouchFragment);
		that.getView().setModel(model, "keepInTouchFormModel");
		that.getView().setModel("keepInTouchFormModel").updateBindings(true);
	}
