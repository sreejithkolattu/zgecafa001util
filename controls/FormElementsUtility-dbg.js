		...
		try {
			...
			if (oModel !== undefined) {
				...
				if (oModel.getData().EmpInfo.length < 1) {
					getModelData("/sap/opu/odata/sap/ZDD_SH_PERSON_CDS/ZDD_SH_PERSON?$filter=substringof('',pernr) and substringof('',ename) and substringof('',srch_email) and substringof('',orgeh) and substringof('',werks) and substringof('',btrtl)", that);
				}
			}
		} catch (e) {
			sap.m.MessageToast.show("Error !!!");
