		...
		try {
			...
			if (oModel !== undefined) {
				...
				if (oModel.getData().EmpInfo.length < 1) {
					getModelData("/sap/opu/odata/sap/ZDD_SH_PERSON_CDS/ZDD_SH_PERSON?$select=pernr,ename,srch_email&$filter=substringof('',pernr) and substringof('',ename) and substringof('',srch_email) and substringof('',orgeh) and substringof('',werks) and substringof('',btrtl)", that);
				}
			}
		} catch (e) {
			sap.m.MessageToast.show("Error !!!");
	...
	function getFilterbar(oValueHelpDialog) {
		try {
			var oFilterBar = new sap.ui.comp.filterbar.FilterBar({
				...
				filterGroupItems: [
					getFilterItem("empGrp", "gn1", "n1", "Personel Number", "searchInput1", "Search Personel Number", "/EmpInfo", "{pernr}",
						"{pernr}"),
					getFilterItem("empGrp", "gn1", "n2", "Employee Name", "searchInput2", "Search Employee Name", "/EmpInfo", "{ename}", "{ename}"),
					/*getFilterItem("empGrp", "gn1", "n3", "First Name", "searchInput3", "Search First Name", "/EmpInfo", "{vorna}", "{vorna}"),
					getFilterItem("empGrp", "gn1", "n4", "Last Name", "searchInput4", "Search Last Name", "/EmpInfo", "{nachn}", "{nachn}"),*/
					...
					/*getFilterItem("empGrp", "gn1", "n6", "Organizational Unit", "searchInput6", "Search Organizational Unit", "/EmpInfo", "{oregh}",
						"{oregh} - {OrgUnitDescription}"),*/
				...
				search: function (oEvt) {
					...
					/*var oVorna = oParams[2].getValue();
					var oNachn = oParams[3].getValue();*/
					...
					var oFilter1 = [new sap.ui.model.Filter("pernr", "Contains", oPernr), new sap.ui.model.Filter("ename", "Contains", oEname),
						/*new sap.ui.model.Filter("vorna", "Contains", oVorna), new sap.ui.model.Filter("nachn", "Contains", oNachn),*/
						new sap.ui.model.Filter("srch_email", "Contains", oEmail),
