			...
			try {
				if (isEmptyValue(empNo)) {
					if (!validateQuickView(that, empNo)) {
						//if(!(that._oQuickView !== undefined && that._oQuickView.getModel("empQuickView").getData().oEmpInfoData !== undefined && that._oQuickView.getModel("empQuickView").getData().oEmpInfoData[0].pageId === empNo)){
						if (that._oQuickView) {
		...
		try {
			...
			if (oModel !== undefined) {
				...
				if (oModel.getData().EmpInfo.length < 1) {
					getModelData("/sap/opu/odata/sap/ZDD_SH_PERSON_CDS/ZDD_SH_PERSON", that);
				}
			}
		} catch (e) {
			sap.m.MessageToast.show("Error !!!");
		...
		try {
			var oFilterBar = new sap.ui.comp.filterbar.FilterBar({
				...
				search: function (oEvt) {
					...
					var oEmail = oParams[4].getValue();
					var oOrgeh = oParams[5].getValue();
					var oWerks = oParams[6].getValue();
					var oBtrtl = oParams[7].getValue();
