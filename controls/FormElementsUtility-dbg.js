sap.ui.define(["jquery.sap.global", "sap/ui/model/Filter", "sap/ui/comp/filterbar/FilterBar", "sap/ui/model/FilterOperator",
	...
	return {
		...
		handleUploadPopover: function (oEvent, that, model) {
			try {
				// if (model) {
				...
				that._oInfoUploadPopover.setModel(model); // Set the employee information

				// attaching the event on close
				sap.ui.core.Fragment.byId("InfoUploadPopover", "btnOk").attachPress(
				...
				var oButton = oEvent.getSource();
				jQuery.sap.delayedCall(0, that, function () {
					that._oInfoUploadPopover.openBy(oButton);
				});
