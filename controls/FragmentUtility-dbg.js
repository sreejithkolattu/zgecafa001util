sap.ui.define(["jquery.sap.global", "sap/ui/core/Control"], function (jQuery, Control) {
	...
	return {
		...
		handleUploadPopover: function (fragInst, model, fragmentId) {
			var that = fragInst;
			...
			that._oDialog = sap.ui.xmlfragment(that.createId(fragmentId), "zgecafa001util.zgecafa001util.fragments.GenericUpload", that);
			that.getView().addDependent(that._oDialog);
			jQuery.sap.syncStyleClass("sapUiSizeCompact", that.getView(), that._oDialog);
			that._oDialog.setModel(model);
			that._oDialog.openBy();
