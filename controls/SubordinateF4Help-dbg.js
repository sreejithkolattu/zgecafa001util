sap.ui.define([
...
], function (jQuery, Filter, FilterBar, FilterOperator, ValueHelpDialog, JSONModel, FragmentUtility) {
	"use strict";
	/**
	...
	function showValueHelpToDialog(that, f4h) {
		try {
				...
				ok: function (oControlEvent) {
					...
					if (!f4h.autoFill) {
						try {
							...
							sap.m.MessageToast.show("Error while choosing Subordinate");
						}
			...
			onValueHelpRequest(null, oValueHelpDialog, that);
	...
	function setFilterSearch(that, oValueHelpDialog, reqestedField) {
		try {
			...
			oFilterBar = getFilterbar(oValueHelpDialog);
	 ...
	 * @public
	 */
	function onValueHelpRequest(oFilter, oValueHelpDialog, that) {
		try {
			oValueHelpDialog.setBusy(true);
			var modelPath = "/sap/opu/odata/sap/ZGEPMGS002_SRV",
				sPath = "SubordinatesSet",
				oModel = new sap.ui.model.odata.ODataModel(modelPath, false);
			oModel.read(sPath, {
				success: jQuery.proxy(handleSuccessPernrData, that),
				error: jQuery.proxy(handleErrorPernrData, that)
			});
		} catch (e) {
			sap.m.MessageToast.show("Error while retrieving the list of Subordinates ");
	 ...
	 * @public
	 */
	function handleSuccessPernrData(oData, oResponce) {
		try {
			var that = this,
				oVhJsonModel = new JSONModel({
					items: oData.results
				});
			that.vhDialog.getTable().setModel(getColumnModel(), "columns");
			that.vhDialog.getTable().setModel(getRowModel());
			that.vhDialog.getTable().setModel(oVhJsonModel);
			that.vhDialog.getTable().bindRows({
				path: "/items"
			});
			that.vhDialog.setBusy(false);
		} catch (e) {
			sap.m.MessageToast.show("Error while retrieving the list of Subordinates");
			that.vhDialog.setBusy(false);
		}
	 ...
	 * @public
	 */
	function handleErrorPernrData(oData, oResponce) {
		try {
			var that = this,
				sErrMessageXML = oData.response.body,
				sErrMessageText = jQuery.parseXML(sErrMessageXML).querySelector("message").textContent,
				oErrorMsgModel = new JSONModel({
					"Title": sErrMessageText,
					"SubTitle": "Could not view the Subordinates",
					"BtnOkText": "OK"
				});
			that.vhDialog.setBusy(false);
			that.vhDialog.close();
			FragmentUtility.handleErrorDialog(that, oErrorMsgModel, "FBFORM_ID_ID");
		} catch (e) {
			that.vhDialog.setBusy(false);
			that.vhDialog.close();
			sap.m.MessageToast.show("Error while retrieving the list of Subordinates");
	...
	function handleException(oError) {
		try {
			...
			if (oError.message.indexOf("adding element with duplicate id") !== -1) {
				if (sap.ui.getCore().byId(oError.message.split("'")[1]) !== undefined) {
					sap.ui.getCore().byId(oError.message.split("'")[1]).destroy();
				}
			}
		} catch (oErr) {
	...
	function getFilterItem(groupTitle, groupName, name, label, controlName, placeholder, aggPath, key, text) {
		try {
			...
			sap.m.MessageToast.show("Error while searching Subordinates");
		}
	}
	/**
				...
				search: function (oEvt) {
					var oParams = oEvt.getParameter("selectionSet"),
						...
						oFilter1 = [new sap.ui.model.Filter("Pernr", "Contains", oPernr),
							new sap.ui.model.Filter("Cname", "Contains", oCname),
