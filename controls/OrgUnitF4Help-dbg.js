sap.ui.define(["jquery.sap.global", "sap/ui/model/Filter", "sap/ui/comp/filterbar/FilterBar", "sap/ui/model/FilterOperator",
	 ...
	 *  1) Organization Unit Search Help
	 */
	return {
		...
		try {
			...
			if (oModel !== undefined) {
				if (oModel.getData().EmpInfo.length < 1) {
					...
					// added by pankaj to pass the present date as parameter 
					// requested by Vinay Joshi to pass the filter parameter as current date
					var oDateFormat = sap.ui.core.format.DateFormat.getInstance({
						pattern: "yyyyMMdd"
					});
					var filterCurrDate = oDateFormat.format(new Date());
					getModelData("/sap/opu/odata/sap/ZGEOM_C_ORG_VALUEHELP_CDS/ZGEOM_C_ORG_VALUEHELP('" + filterCurrDate + "')/Set", that);
					//end of change
				}
			}
		} catch (e) {
	...
	function onValueHelpRequest(oFilter, that) {
		try {
			...
			var sPath = "ZGEOM_C_ORG_VALUEHELP('" + filterCurrDate + "')/Set";
			//end of change
		...
		try {
			var oFilterBar = new sap.ui.comp.filterbar.FilterBar({
				...
				search: function (oEvt) {
					...
					var aFilter = [];
					if (oOrgId) {
						aFilter.push(new sap.ui.model.Filter("OrgUnit", "Contains", oOrgId));
					}
					if (oOrgDesc) {
						aFilter.push(new sap.ui.model.Filter("OrgUnitDesc", "Contains",
							oOrgDesc));
					}

					onValueHelpRequest(aFilter, oValueHelpDialog);
				}
			});
			return oFilterBar;
		} catch (e) {
