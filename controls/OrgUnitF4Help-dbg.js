	...
	return {
		...
		showValueHelpDialog: function (that, f4h) {
			try {
				...
				f4h.title = !isEmptyValue(f4h.title) ? "Org ID" : f4h.title;
				f4h.supportMultiselect = !isEmptyValue(f4h.supportMultiselect) ? true : false;
				f4h.isFragment = (f4h.isFragment !== "" && f4h.isFragment) ? true : false;
				f4h.getToken = (f4h.getToken !== "" && f4h.getToken) ? true : false;
				if (that.myDialog !== undefined) {
					that.myDialog.destroy();
				}
				that.myDialog = showValueHelpToDialog(that, f4h);
			} catch (e) {
				handleException(e);
			}
		}
